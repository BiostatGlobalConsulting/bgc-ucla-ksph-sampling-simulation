...
program define ucla_ksph_sim1
	...
	********************************************************************************
	...
	local svyset3 bclusterid, singleunit(scaled)
	...
	forvalues np = 1/5 {					// np = npsus index ... see list above for the list of npsus values
		...
		quietly {
			...
			forvalues runid = 1/20 {		// draw 20 samples at each value of npsus
				...
				if `np' < 5 {				// the first 5 schemes are cluster samples
				...
				else sample `npsus', count	// the 6th scheme is a simple random sample
	...
	* median and range of several interesting parameters.
	...
	label var np "N PSUs"
	label define npsus 1 "5" 2 "10" 3 "20" 4 "41" 5 "103" , modify
	...
	forvalues i = 1/5 {
		...
		sum deff if np == `i',d
		local string`i' `string`i'' `=string(r(p50),"%5.1f")' (`=string(r(min),"%5.1f")'-`=string(r(max),"%5.1f")'); NEFF:
		sum neff if np == `i',d
	...
	* Combine plots and export to png
	...
	graph export plot_`abbrev_dataset'_`abbrev_outcome'_`stratum'.png, width(2000) replace
