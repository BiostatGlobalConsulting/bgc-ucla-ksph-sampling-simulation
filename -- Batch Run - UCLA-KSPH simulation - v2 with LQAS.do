...
*
* This program demonstrates running the sim2 program on that output.
*
*
...
foreach s in `slist' {
	ucla_ksph_sim2, dataset(TPVICS_R2_ADS) outcome(fully_vaccinated_crude)      stratum(`s') abbrev_dataset(tpvr2) abbrev_outcome(fvc)    title_suffix(- Fully Vxd)
	ucla_ksph_sim2, dataset(TPVICS_R2_ADS) outcome(not_vaccinated_crude)        stratum(`s') abbrev_dataset(tpvr2) abbrev_outcome(nvc)    title_suffix(- Zero Dose)
}
...
levelsof stratumid, local(slist)
foreach s in `slist' {
	ucla_ksph_sim2, dataset(BFASO_2016_RI_ADS) outcome(fully_vaccinated_crude)      stratum(`s') abbrev_dataset(bfaso16) abbrev_outcome(fvc)    title_suffix(- Fully Vxd)
	ucla_ksph_sim2, dataset(BFASO_2016_RI_ADS) outcome(not_vaccinated_crude)        stratum(`s') abbrev_dataset(bfaso16) abbrev_outcome(nvc)    title_suffix(- Zero Dose)
}

foreach o in y01 y50a y50b y50c y75a y75b y75c y99 y100 {
	ucla_ksph_sim2, dataset(survey_with_plausible_yet_perverse_proportions) outcome(`o') stratum(1) abbrev_dataset(faux) abbrev_outcome(`o') title_suffix(- Faux Outcome `o')
}
