	...
	* because of the value of the global macro: VCQI_LOGOPEN.
	...
	quietly {
		...
		* establish default values if the user doesn't specify them
		if "`ylabel'" == "" local ylabel 0(50)100, angle(h) nogrid labsize(vsmall)
		...
		* ignore footnote
		...
		if "`ytitle'" != "" local pass_thru `pass_thru' ytitle("`ytitle'", size(vsmall))
		...
		* set up the twoway line and twoway scatteri syntax to do it.
		if "`plotn'" != "" {
			...
			local yaxis2title ytitle("`ytitle2'", axis(2) size(vsmall))
			local yaxis2label ylabel(0(`yround2')`y2max', axis(2) angle(0) labsize(vsmall))
		}
